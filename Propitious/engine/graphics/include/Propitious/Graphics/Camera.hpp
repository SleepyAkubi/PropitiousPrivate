...
namespace Propitious
{
	...
	struct PROPITIOUS_GRAPHICS_EXPORT Camera
	{
	...
	PROPITIOUS_GRAPHICS_EXPORT void lookAt(Camera& camera, const Vector3& position, const Vector3& up = Vector3{ 0, 1, 0 });
	PROPITIOUS_GRAPHICS_EXPORT void offsetOrientation(Camera& camera, const Radian& yaw, const Radian& pitch);

	PROPITIOUS_GRAPHICS_EXPORT Matrix4 matrix(const Camera& camera);
	PROPITIOUS_GRAPHICS_EXPORT Matrix4 projection(const Camera& camera);
	PROPITIOUS_GRAPHICS_EXPORT Matrix4 view(const Camera& camera);
