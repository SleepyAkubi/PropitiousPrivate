...
namespace Propitious
{
	namespace Types
	{
		bool loadBMP(Image& image, const a8* path, FileSystem& filesys)
		{
			...
			FILE* file = filesys.loadFile(path);
			if (!file)
			{
				return false;
			}
	...
	bool loadFromFile(Image& image, const a8* path, FileSystem& filesys)
	{
		return Types::loadBMP(image, path, filesys);
	}
