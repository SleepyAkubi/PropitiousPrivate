...
namespace Propitious
{
		...
		const WindowStyle style)
	{
		...
		String iconPath = getPathToExe() + "\\game.ico";
		const a8* cIconPath = unwiden(cString(iconPath));

		HICON gameIconSmall = (HICON)LoadImageA(
			NULL,
			cIconPath,
			IMAGE_ICON,
			16,
		...
		windowHandle->handle = CreateWindowA(
			cWindowClass,
			cTitle,
		...
		HICON gameIcon = (HICON)LoadImageA(
			NULL,
			cIconPath,
			IMAGE_ICON,
			64,
	...
	{
		WindowHandle* windowHandle = (WindowHandle*)extraData;
		SetWindowText(windowHandle->handle, cString(title));
	}
