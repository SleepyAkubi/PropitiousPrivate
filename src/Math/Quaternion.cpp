...
namespace Propitious {
  ...
  Quaternion normalize(const Quaternion& q)
  {
    ...
    {
        return atan2(2.0f * q.data[1] * q.data[2] + q.data[3] * q.data[0],
                          q.data[3] * q.data[3] - q.data[0] * q.data[0] - q.data[1] * q.data[1] + q.data[2] * q.data[2]);
    }

    Radian yaw(const Quaternion& q)
    {
        return asin(-2.0f * (q.data[0] * q.data[2] - q.data[3] * q.data[1]));
    }

    Radian roll(const Quaternion& q)
    {
        return atan2(2.0f * q.data[0] * q.data[1] + q.data[2] * q.data[3],
