	...
	namespace OpenGL
	{
		...
		namespace System
		{
			namespace
			{
				...
				struct MapEntry
				{
					MapEntry(const char *_extName, Extentions::LoadTest *_extVariable)
						: extName(_extName)
						, extVariable(_extVariable)
						, loaderFunc(0)
					{}

					MapEntry(const char *_extName, Extentions::LoadTest *_extVariable, PFN_LOADEXTENSION _loaderFunc)
						: extName(_extName)
						, extVariable(_extVariable)
					...
					const char *extName;
					Extentions::LoadTest *extVariable;
				...
				void LoadExtByName(std::vector<MapEntry> &table, const char *extensionName)
				{
					...
					{
						if (entry->loaderFunc)
							(*entry->extVariable) = Extentions::LoadTest(true, entry->loaderFunc());
						else
							(*entry->extVariable) = Extentions::LoadTest(true, 0);
					}
				}
			} //namespace 
			...
			namespace
			{
				static void ProcExtsFromExtList(std::vector<MapEntry> &table)
				{
					...
					OpenGL::_detail::GetIntegerv(OpenGL::NUM_EXTENSIONS, &iNumExtensions);

					for (iLoop = 0; iLoop < iNumExtensions; iLoop++)
					{
						const char *strExtensionName = (const char *)OpenGL::_detail::GetStringi(OpenGL::EXTENSIONS, iLoop);
