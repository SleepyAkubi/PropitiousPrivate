...
namespace Propitious
{
	GameClient::GameClient(const std::string& gamePath)
	{
		...
		Propitious::initialise(InitOpenGL);
		...
		renderContext.textureHolder->setPath(gamePath);
		renderContext.meshHolder->setPath(gamePath);
		renderContext.shaderHolder->setPath(gamePath);
	...
	void GameClient::run()
	{
		...
		while (1)
		{
			...
			{
				window.setTitle(
					gameinfo.name + " | Debug - " + std::to_string((i32)tc.tickRate) + "fps | " + std::to_string(1000.0f / tc.tickRate) + (" ms"));
			}
#endif
